...
model = Sequential()
model.add(Conv2D(filters=64, kernel_size=[7,7], padding='same', kernel_initializer = initializers.he_normal(seed=1), activation="relu", input_shape=(128,128,1)))
# Dim = (128x128x64)
model.add(BatchNormalization())
model.add(AveragePooling2D(pool_size=[2,2], strides=2))
# Dim = (64x64x64)
model.add(Conv2D(filters=128, kernel_size=[7,7], padding='same', strides=2, kernel_initializer = initializers.he_normal(seed=1), activation="relu"))
# Dim = (32x32x128)
model.add(BatchNormalization())
